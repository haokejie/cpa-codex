...
import type {
  ...
  QuotaAntigravityQuotaInfo,
  QuotaAntigravityModelsPayload,
  QuotaGeminiCliParsedBucket,
  QuotaGeminiCliQuotaBucketState,
  QuotaKimiUsagePayload,
  QuotaKimiUsageDetail,
  QuotaKimiLimitItem,
  QuotaKimiLimitWindow,
  QuotaKimiQuotaRow,
} from '../../types';
import {
...
export function buildGeminiCliQuotaBuckets(
  buckets: QuotaGeminiCliParsedBucket[]
): QuotaGeminiCliQuotaBucketState[] {
  ...
  type GeminiCliQuotaBucketGroup = {
    ...
    preferredBucket?: QuotaGeminiCliParsedBucket;
...
export function getAntigravityQuotaInfo(entry?: QuotaAntigravityQuotaInfo): {
...
export function findAntigravityModel(
  models: QuotaAntigravityModelsPayload,
  identifier: string
): { id: string; entry: QuotaAntigravityQuotaInfo } | null {
...
export function buildAntigravityQuotaGroups(
  ...
  const groups: QuotaAntigravityQuotaGroup[] = [];
  ...
  ): QuotaAntigravityQuotaGroup | null => {
    const matches = def.identifiers
      .map((identifier) => findAntigravityModel(models, identifier))
      .filter((entry): entry is { id: string; entry: QuotaAntigravityQuotaInfo } => Boolean(entry));
...
type KimiRowLabel = Pick<QuotaKimiQuotaRow, 'label' | 'labelKey' | 'labelParams'>;
...
function kimiLimitLabel(
  item: QuotaKimiLimitItem,
  detail: QuotaKimiUsageDetail | QuotaKimiLimitItem,
...
export function buildKimiQuotaRows(payload: QuotaKimiUsagePayload): QuotaKimiQuotaRow[] {
  const rows: QuotaKimiQuotaRow[] = [];
  ...
  if (Array.isArray(limits)) {
    limits.forEach((item, idx) => {
      const detail = (item.detail && typeof item.detail === 'object' ? item.detail : item) as QuotaKimiUsageDetail | QuotaKimiLimitItem;
      const window = (item.window && typeof item.window === 'object' ? item.window : {}) as QuotaKimiLimitWindow;
