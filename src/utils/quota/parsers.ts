...
import type {
  QuotaClaudeUsagePayload,
  QuotaCodexUsagePayload,
  QuotaGeminiCliQuotaPayload,
  QuotaKimiUsagePayload,
} from '../../types';
import { normalizeAuthIndex } from '../usage';
...
export function parseClaudeUsagePayload(payload: unknown): QuotaClaudeUsagePayload | null {
  if (payload === undefined || payload === null) return null;
  if (typeof payload === 'string') {
    const trimmed = payload.trim();
    if (!trimmed) return null;
    try {
      return JSON.parse(trimmed) as QuotaClaudeUsagePayload;
    } catch {
      return null;
    }
  }
  if (typeof payload === 'object') {
    return payload as QuotaClaudeUsagePayload;
  }
  return null;
}

export function parseCodexUsagePayload(payload: unknown): QuotaCodexUsagePayload | null {
  if (payload === undefined || payload === null) return null;
  if (typeof payload === 'string') {
    const trimmed = payload.trim();
    if (!trimmed) return null;
    try {
      return JSON.parse(trimmed) as QuotaCodexUsagePayload;
    } catch {
      return null;
    }
  }
  if (typeof payload === 'object') {
    return payload as QuotaCodexUsagePayload;
  }
  return null;
}

export function parseGeminiCliQuotaPayload(payload: unknown): QuotaGeminiCliQuotaPayload | null {
  if (payload === undefined || payload === null) return null;
  if (typeof payload === 'string') {
    const trimmed = payload.trim();
    if (!trimmed) return null;
    try {
      return JSON.parse(trimmed) as QuotaGeminiCliQuotaPayload;
    } catch {
      return null;
    }
  }
  if (typeof payload === 'object') {
    return payload as QuotaGeminiCliQuotaPayload;
  }
  return null;
}

export function parseKimiUsagePayload(payload: unknown): QuotaKimiUsagePayload | null {
  if (payload === undefined || payload === null) return null;
  if (typeof payload === 'string') {
    const trimmed = payload.trim();
    if (!trimmed) return null;
    try {
      return JSON.parse(trimmed) as QuotaKimiUsagePayload;
    } catch {
      return null;
    }
  }
  if (typeof payload === 'object') {
    return payload as QuotaKimiUsagePayload;
  }
  return null;
}
