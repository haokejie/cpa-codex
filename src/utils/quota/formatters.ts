...
import type { QuotaCodexUsageWindow } from '../../types';
...
export function formatCodexResetLabel(window?: QuotaCodexUsageWindow | null): string {
  if (!window) return '-';
...
export function formatKimiResetHint(t: TranslateFunc | undefined, hint?: string): string {
  if (!hint) return '';
  if (t) return t('kimi_quota.reset_hint', { hint });
  return `重置：${hint}`;
}
