...
import { buildHeaderObject } from './headers';
 ...
 * 规范化 /config 返回值
 */
export const normalizeConfigResponse = (raw: unknown): ServerConfig => {
  const config: ServerConfig = { raw: isRecord(raw) ? raw : {} };
  if (!isRecord(raw)) {
    return config;
  }
  ...
  const ampcode = normalizeAmpcodeConfig((raw as Record<string, unknown>).ampcode);
  if (ampcode) {
