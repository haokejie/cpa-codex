...
import { DEFAULT_VISUAL_VALUES } from "../types";
...
export function useVisualConfig() {
  ...
  const visualDirty = computed(
    () => JSON.stringify(visualValues.value) !== JSON.stringify(baselineValues.value),
  );

  const loadVisualValuesFromYaml = (yamlContent: string) => {
    try {
  ...
  const applyVisualChangesToYaml = (currentYaml: string): string => {
    try {
      ...
      if (values.apiKeysText !== baseline.apiKeysText) {
  ...
  const setVisualValues = (newValues: Partial<VisualConfigValues>) => {
    visualValues.value = {
      ...visualValues.value,
      ...newValues,
      streaming: {
